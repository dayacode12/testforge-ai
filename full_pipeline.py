...
Format your response clearly with bullet points."""
    ...
    try:
        ...
        return "Could not connect to Ollama at http://ollama:11434. Is it running?"
    except requests.exceptions.Timeout:
        return "Ollama took too long to respond (timeout after 300 seconds)"
    except Exception as e:
...
def send_to_testforge_for_test_generation(feature):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"TestForge returned error {response.status_code}"
    except requests.exceptions.ConnectionError:
        return "Could not connect to testforge-ai at http://test-awesome:8000. Is it running?"
    except requests.exceptions.Timeout:
        return "TestForge took too long to respond (timeout after 300 seconds)"
    except Exception as e:
...
def display_fixes(analysis):
    ...
    print("\n" + "█"*75)
    print("█ OLLAMA AI RECOMMENDATIONS")
    print("█"*75)
    print(analysis)
    print("█"*75 + "\n")
